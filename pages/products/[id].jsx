...
import ThumbsCarousel from 'components/reuseable/ThumbsCarousel'; // -------- hook -------- //
        ...
        <div className="container py-14 py-md-16">
            ...
            <div className="col-lg-6">
              ...
              <form onSubmit={handleFormSubmit}>
                <fieldset className="picker">
                  ...
                  {data.sizeList.map(({
                    ...
                    title
                      ...
                      <input type="radio" name="sizes" value={value} id={`size-${value}`} checked={value === size} onChange={e => setSize(e.target.value)} />
                      <span>{title}</span>
                    </label>)}
                </fieldset>
                ...
                <fieldset className="picker">
                  ...
                  {data.productColors.map(({
                    id,
                    value
                  }) => <label htmlFor={id} key={id}>
                      <input id={id} type="radio" name="colors" value={value} checked={value === color} onChange={e => setColor(e.target.value)} />
                      <span style={{
                        color: value
                      }}>Yellow</span>
                    </label>)}
                    ...
                    <div>
                      <div className="form-select-wrapper">
                        <select value={quantity} className="form-select" onChange={e => setQuantity(+e.currentTarget.value)}>
                          {[1, 2, 3, 4, 5].map(item => <option key={item} value={item}>
                            {item}
                          </option>)}
                        </select>
                      </div>
          ...
          <ul className="nav nav-tabs nav-tabs-basic mt-12">
            {data.tabList.map(({
              id,
              title
            }, i) => <li className="nav-item" key={id}>
        ...
        <div className="container py-14 py-md-16">
          ...
          <div className="swiper-container blog grid-view shop mb-6">
            <Carousel navigation={false} breakpoints={carouselBreakpoints}>
              {products.slice(0, 5).map(item => <ProductCard {...item} key={item.id} />)}
            </Carousel>
          </div>
        </div>
      </section>
            ...
            <div className="col-lg-8">
              ...
              <div id="comments">
                <ol id="singlecomments" className="commentlist">
                  {data.reviewList.map(item => <ReviewCard key={item.id} {...item} />)}
                </ol>
              </div>
